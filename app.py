...
st.markdown("<h1 style='margin-top:0.25rem;'>Welcome to the Customer Value Analytics Hub</h1>", unsafe_allow_html=True)
st.markdown(
    "<h4>Developed by <a href='https://www.linkedin.com/in/qazi-maria-anees/' "
    "target='_blank' style='color:pink; text-decoration:underline;'> Qazi Maria</a></h4>",
    unsafe_allow_html=True
)
...
left, right = st.columns([1, 3], gap="large")  # 1:3 layout

with left:
    with st.expander("📌 About this App", expanded=False):
        st.markdown("""
        This application is designed to help businesses **understand and predict customer spending behavior**.  
        It leverages transactional data to extract RFM (Recency, Frequency, Monetary) features and applies either **heuristic rules** or **machine learning models** to generate insights.
        """)

    with st.expander("⚙️ How to Use", expanded=False):
        st.markdown("""
        1. **Upload Data** → Import your transactional CSV/Excel file.  
        2. **Clean & Process** → The app automatically preprocesses data.  
        3. **Analyze Customers** → View RFM scores and spending predictions.  
        4. **Filter & Segment** → Use sidebar filters to focus on specific customer groups.  
        5. **Visualize** → Explore graphs, charts, and tables for customer insights.  
        6. **Download Results** → Export processed data and reports for further use.  
        """)
...
# ---------- Calibration + Brier Score (compact, fits left column) ----------
# ---------- Calibration + Brier Score (fits blue box under the scatter) ----------
with right:  # render directly beneath the main scatter in the right column
    st.markdown("Calibration (model vs perfect)")
    ...
    if "spend_90_total" in df_customers.columns and "pred_prob" in df_customers.columns:
        try:
            ...
            # Compact Plotly figure sized for the right-column box
            fig_cal = go.Figure()
            fig_cal.add_trace(
                go.Scatter(x=prob_pred, y=prob_true, mode="lines+markers",
                           name="Model", line=dict(color="#2F80ED"))
            )
            fig_cal.add_trace(
            ...
            fig_cal.update_layout(
                ...
                yaxis_title="Actual Return Rate",
            ...
            # --- Brier Score (green if model better than baseline, else red) ---
            brier = brier_score_loss(y_true, y_pred)
            p_hat = float(np.mean(y_true))
            brier_baseline = np.mean((y_true - p_hat) ** 2)

            score_color = "green" if brier < 0.25 else "red"


            st.markdown(
                f"<span style='color:white;'>Brier Score: </span>"
                f"<span style='color:{score_color};'>{brier:.4f}</span> "
